...
import { createChunk } from './chunking-strategies';

// --- STATE ---
let provider: LLMProvider | null = null;
let statusCallback: ((status: RagStatus, message?: string) => void) | null = null;
let vectorStore: VectorStoreItem[] = [];
let db: IDBDatabase | null = null;
let status: RagStatus = 'idle';
...
function updateStatus(newStatus: RagStatus, message?: string) {
    status = newStatus;
    if (statusCallback) {
...
export async function init(llmProvider: LLMProvider, callback: (status: RagStatus, message?: string) => void) {
...
export async function buildStore() {
    ...
    const chunks: { chunk: string, metadata: { source: string, name: string } }[] = [];
    ...
    try {
        ...
        embeddings.forEach((embedding, i) => {
            const item: Omit<VectorStoreItem, 'id'> = {
...
export async function search(query: string, topK: number = 3): Promise<VectorStoreItem[]> {
