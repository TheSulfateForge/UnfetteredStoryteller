...
import { generateBaseRpgPlayerState, getPointBuyCost, getStartingEquipment, getEquipmentFromBackground, getAbilityModifierValue, calculateArmorClass, getClassFeaturesForLevel } from './rpg-helpers';
import { gameState } from './state-manager';
import { CharacterInfo, PlayerState, SaveSlot, AbilityScores } from './types';
import * as game from './game';
import { startAdventure } from './game-loop';
import { initializeChatSession } from './session-manager';
import { toCamelCase } from './utils';

// --- STATE & CONSTANTS ---

const STAT_ABBREVIATIONS: { [key in keyof AbilityScores]: string } = {
    strength: 'str',
    dexterity: 'dex',
    constitution: 'con',
    intelligence: 'int',
...
export function updateSkillSelectionUI() {
    ...
    const allSkillChoices: { source: string, choose: number, from: string[], groupName: string }[] = [];
    ...
    sources.forEach((source, sourceIndex) => {
        source.data?.skill_proficiencies?.forEach((prof: any) => {
            if (prof.type === 'granted') {
                prof.from.forEach((skillName: string) => grantedSkills.add(skillName));
...
export function updateCheckboxStates() {
    ...
    container.querySelectorAll<HTMLInputElement>('input[type="checkbox"][id^="skill-choice-"]:checked').forEach(cb => {
        allProficientSkills.add(cb.value);
    });

    const allChoiceCheckboxes = Array.from(container.querySelectorAll<HTMLInputElement>('input[type="checkbox"][id^="skill-choice-"]'));
...
export function updatePointBuyUI() {
    ...
    for (const [stat, score] of Object.entries(pointBuyState.scores)) {
        const statAbbr = STAT_ABBREVIATIONS[stat as keyof AbilityScores];
        (document.getElementById(`${stat}-score`) as HTMLElement).textContent = String(score);
        (document.getElementById(`${stat}-cost`) as HTMLElement).textContent = String(getPointBuyCost(score));
        (document.getElementById(`${statAbbr}-decrease`) as HTMLButtonElement).disabled = (score <= 8);
        (document.getElementById(`${statAbbr}-increase`) as HTMLButtonElement).disabled = (score >= 20);
    }
}

/** Handles increasing or decreasing an ability score. */
export function updateAbilityScore(stat: keyof typeof pointBuyState.scores, direction: 'increase' | 'decrease') {
...
export async function handleCharacterCreationSubmit(event: Event) {
    ...
    const characterInfo: CharacterInfo = {
        ...
        gender: dom.charGenderInput.value as 'male' | 'female'
    };
    ...
    for (const key in finalState.abilityScores) {
        const baseScore = pointBuyState.scores[key as keyof typeof pointBuyState.scores];
        finalState.abilityScores[key as keyof AbilityScores] = baseScore + (raceData.ability_bonuses?.[key] || 0);
    }
    ...
    (classData.prof_saving_throws || "").toLowerCase().split(', ').forEach(save => {
        if (save in finalState.savingThrows) finalState.savingThrows[save as keyof typeof finalState.savingThrows] = 'proficient';
    });
    ...
    document.querySelectorAll<HTMLInputElement>('#skill-selection-container input[type="checkbox"]:checked').forEach(checkbox => {
        const skillKey = toCamelCase(checkbox.value);
        if (skillKey in finalState.skills) finalState.skills[skillKey as keyof typeof finalState.skills] = 'proficient';
    });
    ...
    try {
        ...
        const storyHooks = Array.isArray(result) ? result : ((result as any).storyHooks || []);
...
export function handleCustomHookSubmit(event: Event) {
    event.preventDefault();
