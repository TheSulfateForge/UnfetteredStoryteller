...
import { PlayerState, CharacterInfo, AbilityScores } from './types';
import * as dataManager from './data-manager';

const SKILL_TO_ABILITY_MAP: Record<string, keyof AbilityScores> = {
...
export const DEFAULT_SKILLS: PlayerState['skills'] = {
...
export const DEFAULT_SAVING_THROWS: PlayerState['savingThrows'] = {
 ...
 * @param score The ability score value (from 8 to 20).
 * @returns The number of points required for that score.
 */
...
export function getAbilityModifierValue(score: number): number {
...
export function getWeaponData(weaponName: string): dataManager.Weapon | null {
...
export function calculateArmorClass(playerState: PlayerState): number {
...
export function calculateRollModifier(skillOrAbility: string, playerState: PlayerState): number {
    const cleanSkillOrAbility = skillOrAbility.toLowerCase().replace(/\s/g, '');
    const abilityKey = SKILL_TO_ABILITY_MAP[cleanSkillOrAbility as keyof typeof SKILL_TO_ABILITY_MAP];
    ...
    if (Object.keys(playerState.skills).includes(cleanSkillOrAbility) && playerState.skills[cleanSkillOrAbility as keyof typeof playerState.skills] === 'proficient') {
        proficiencyBonus = playerState.proficiencyBonus;
    }
    else if (Object.keys(playerState.savingThrows).includes(abilityKey) && playerState.savingThrows[abilityKey as keyof typeof playerState.savingThrows] === 'proficient') {
        proficiencyBonus = playerState.proficiencyBonus;
    }
...
export function generateBaseRpgPlayerState(charInfo: CharacterInfo): PlayerState {
    const level = 1;
    const proficiencyBonus = calculateProficiencyBonus(level);
    const abilityScores: AbilityScores = { strength: 10, dexterity: 10, constitution: 10, intelligence: 10, wisdom: 10, charisma: 10 };
 ...
 */
export function getClassFeaturesForLevel(classData: dataManager.Class, level: number): string[] {
 ...
 * @param characterClass The character's class name.
 * @returns An object with equipment and inventory arrays.
 */
export function getStartingEquipment(characterClass: string): { equipment: { weapon: string; armor: string; }; inventory: string[]; } {
 ...
 * @param backgroundData The background data object.
 * @returns An array of item names.
 */
export function getEquipmentFromBackground(backgroundData: dataManager.Background): string[] {
