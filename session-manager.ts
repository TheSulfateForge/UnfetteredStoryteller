...
import { PlayerState, LLMProviderType, ProviderSettings } from './types';
import { Part } from '@google/genai';
import { cleanseResponseText, createLlmProvider } from './api';
import { resetPointBuy } from './character-creator';

// --- HELPERS ---

function isPlayerStateValid(state: any): state is PlayerState {
    if (!state || typeof state !== 'object') return false;
    const has = (prop: string) => Object.prototype.hasOwnProperty.call(state, prop);
...
async function proceedToAdventure(action: 'new' | 'load', setupMainAppEventListeners: () => void) {
...
function handleSettingsSave(e: Event) {
    e.preventDefault();
    const provider = dom.providerSelector.value as LLMProviderType;
...
export function setupInitialEventListeners(setupMainAppEventListeners: () => void) {
...
export function newGame(isMature: boolean) {
    services.tts.cancel();
...
export async function loadGame(characterId: string) {
    ...
    saveSlot.chatHistory.forEach(message => {
        const text = (message.parts as Part[]).map(p => p.text).join('');
